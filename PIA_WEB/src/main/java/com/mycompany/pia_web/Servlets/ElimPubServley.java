 ...
 * @author migue
 */
@WebServlet(name = "ElimPubServley", urlPatterns = {"/ElimPubServley"})
public class ElimPubServley extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ElimPubServley</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ElimPubServley at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       HttpSession session = request.getSession();
           Publicaciones publi1 = (Publicaciones) session.getAttribute("Publicacion");
  int id=publi1.getIdpubli();
   try {
     ...
     Pub1.setIdpubli(id);
        ...
        Conexion conn =  new Conexion();
            ...
            PublicacionDAO uDao = new PublicacionDAO(conn.Conectar());
                
         
           boolean update = uDao.deletePublicacion(Pub1);

            if(update)
            {
                //Metodo para redireccionar a otra vista/Servlet
                request.setAttribute("mensaje", "Publicacion Eliminada Correctamente");
